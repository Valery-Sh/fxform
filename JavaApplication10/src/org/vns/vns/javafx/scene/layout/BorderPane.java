 ...
 */
package org.vns.vns.javafx.scene.layout;
 ...
 *
 * <p>
 * <img src="doc-files/borderpane.png"/> </p>
 *
 * The top and bottom children will be resized to their preferred heights and
 * extend the width of the border pane. The left and right children will be
 * resized to their preferred widths and extend the length between the top and
 * bottom nodes. And the center node will be resized to fill the available space
 * in the middle. Any of the positions may be null.
 *
 * Example:
 ...
 *
 * <p>
 * BorderPane lays out each child set in the five positions regardless of the
 * child's visible property value; unmanaged children are ignored.</p>
 *
 * <h4>Resizable Range</h4>
 * BorderPane is commonly used as the root of a
 * {@link javafx.scene.Scene Scene}, in which case its size will track the size
 * of the scene. If the scene or stage size has not been directly set by the
 * application, the scene size will be initialized to the border pane's
 * preferred size. However, if a border pane has a parent other than the scene,
 * that parent will resize the border pane within the border pane's resizable
 * range during layout. By default the border pane computes this range based on
 * its content as outlined in the table below.
 * <p>
 * <table border="1">
 * <tr><td></td><th>width</th><th>height</th></tr>
 * <tr><th>minimum</th>
 * <td>left/right insets plus width required to display right/left children at
 * their pref widths and top/bottom/center with at least their min widths</td>
 * <td>top/bottom insets plus height required to display top/bottom children at
 * their pref heights and left/right/center with at least their min
 * heights</td></tr>
 * <tr><th>preferred</th>
 * <td>left/right insets plus width required to display
 * top/right/bottom/left/center children with at least their pref widths</td>
 * <td>top/bottom insets plus height required to display
 * top/right/bottom/left/center children with at least their pref
 * heights</td></tr>
 * <tr><th>maximum</th>
 * <td>Double.MAX_VALUE</td><td>Double.MAX_VALUE</td></tr>
 * </table>
 * <p>
 * A border pane's unbounded maximum width and height are an indication to the
 * parent that it may be resized beyond its preferred size to fill whatever
 * space is assigned to it.
 * <p>
 * BorderPane provides properties for setting the size range directly. These
 * properties default to the sentinel value Region.USE_COMPUTED_SIZE, however
 * the application may set them to other values as needed:
 * <pre><code>
 *     <b>borderPane.setPrefSize(500,400);</b>
 * </code></pre> Applications may restore the computed values by setting these
 * properties back to Region.USE_COMPUTED_SIZE.
 * <p>
 * BorderPane does not clip its content by default, so it is possible that
 * childrens' bounds may extend outside its own bounds if a child's min size
 * prevents it from being fit within it space.</p>
 *
 * <h4>Optional Layout Constraints</h4>
 *
 * An application may set constraints on individual children to customize
 * BorderPane's layout. For each constraint, BorderPane provides a static method
 * for setting it on the child.
 * <p>
 * <table border="1">
 * <tr><th>Constraint</th><th>Type</th><th>Description</th></tr>
 * <tr><td>alignment</td><td>javafx.geometry.Pos</td><td>The alignment of the
 * child within its area of the border pane.</td></tr>
 * <tr><td>margin</td><td>javafx.geometry.Insets</td><td>Margin space around the
 * outside of the child.</td></tr>
 * </table>
 * <p>
 * Example:
...
public class BorderPane extends Pane {
    ...
    /**
     * Creates an BorderPane layout with the given Nodes to use for each of the
     * main layout areas of the Border Pane. The top, right, bottom, and left
     * nodes are listed in clockwise order.
     *
     * @param center The node to set as the center of the BorderPane.
    ...
    /**
     * The node placed in the center of this border pane. If resizable, it will
     * be resized fill the center of the border pane between the top, bottom,
     * left, and right nodes. If the node cannot be resized to fill the center
     * space (it's not resizable or its max size prevents it) then it will be
    ...
    /**
     * The node placed on the top edge of this border pane. If resizable, it
     * will be resized to its preferred height and it's width will span the
     * width of the border pane. If the node cannot be resized to fill the top
     * space (it's not resizable or its max size prevents it) then it will be
     * aligned top-left within the space unless the child's alignment constraint
     * has been set.
     */
    ...
    /**
     * The node placed on the bottom edge of this border pane. If resizable, it
     * will be resized to its preferred height and it's width will span the
     * width of the border pane. If the node cannot be resized to fill the
     * bottom space (it's not resizable or its max size prevents it) then it
     * will be aligned bottom-left within the space unless the child's alignment
    ...
    /**
     * The node placed on the left edge of this border pane. If resizable, it
     * will be resized to its preferred width and it's height will span the
     * height of the border pane between the top and bottom nodes. If the node
     * cannot be resized to fill the left space (it's not resizable or its max
     * size prevents it) then it will be aligned top-left within the space
    ...
    /**
     * The node placed on the right edge of this border pane. If resizable, it
     * will be resized to its preferred width and it's height will span the
     * height of the border pane between the top and bottom nodes. If the node
     * cannot be resized to fill the right space (it's not resizable or its max
     * size prevents it) then it will be aligned top-right within the space
     * unless the child's alignment constraint has been set.
     */
    ...
    /**
     * @return null unless the center, right, bottom, left or top has a content
     * bias.
     */
    @Override
    ...
    protected void layoutChildren() {
        ...
        if (c != null && c.isManaged()) {
            Pos alignment = null;//getAlignment(c);
