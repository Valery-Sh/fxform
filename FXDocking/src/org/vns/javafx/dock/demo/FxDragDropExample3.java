...
//        d.start(st2Scene);
    ...
    private void dragDetected(MouseEvent event, ListView<Fruit> listView) {
        ...
        ImageCursor im = new ImageCursor(new Image(
                "https://cdn2.iconfinder.com/data/icons/web/512/Cursor-512.png",
                32, 32, true, true
        ));
        ((Node) event.getSource()).setCursor(Cursor.HAND);
        ((Node) event.getSource()).getScene().setOnDragExited(e->{
            ((Node) event.getSource()).setCursor(Cursor.HAND);
        });
        ((Node) event.getSource()).getScene().setOnDragEntered(e->{
            ((Node) event.getSource()).setCursor(Cursor.HAND);
        });
                
        //listView.getScene().setCursor(im);
