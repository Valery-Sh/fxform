 ...
 */
public class DefaultTopWindowFinder1 implements TopWindowFinder {
    ...
    //private final ObservableMap<Window, WindowWrapper> wrapperMap = FXCollections.observableHashMap();
    ...
    ObservableList<Window> allWindows = FXCollections.observableArrayList();
...
//                    System.err.println("1. alwaysOnTopListener getWrapper(win) = " + getWrapper(win));
//                    System.err.println("1.    --  setLateOntop(wr) = " + wr);
                
                if ( getWrapper(win).isImplicitlyOnTop(false) ) {
//                    System.err.println("2. alwaysOnTopListener getWrapper(win) = " + getWrapper(win));
//                    System.err.println("2.    --  setLateOntop(wr) = " + wr);
    ...
    public static DefaultTopWindowFinder1 getInstance() {
...
//                System.err.println("2. wr = " + wr);
    ...
    private void computeLastFocused(WindowWrapper wrapper) {
        WindowWrapper last = computeLastFocused(wrapper, wrapper.getAllChildren());
//        System.err.println( "wrapper = " + wrapper + "; LAST = " + last);
    ...
    private Window getShowingOwner(Window win) {
        Window owner  = getOwner(win);
        while ( owner != null && ! owner.isShowing()) {
            owner = getOwner(owner);
        }
        return owner;
    }
    private void createChidren(WindowWrapper wrapper) {
        orderedWindows.forEach(w -> {
...
//                child.setLevel(wrapper.getLevel() + 1);
                createChidren(child);
            }
        });
    }

    private WindowWrapper computeLastFocused(WindowWrapper listOwner, List<WindowWrapper> list) {
    ...
    private WindowWrapper getOwnerTree() {
        return ownerTree;
    }
...
//            System.err.println("******** wr = " + wr);
    ...
    /*    private List<Window> getOwnerChain(Window win) {
...
/*        if (retval != null) {
            System.err.println("RETVAL = " + ((Stage) retval).getTitle());
            if (retval instanceof Stage) {
                //((Stage)retval).toFront();
            }
        } else {
...
*/
    ...
    private Window getTopWindow(Window w1, WindowWrapper root1, Window w2, WindowWrapper root2) {
        Window retval = null;
...
//            System.err.println("root2 wr2 = " + wr2);
    ...
    private Window getHigher(Window w1, Window w2) {
    ...
    private Window getHigherWhenBothOnTop(Window w1, Window w2) {
        if (w1.isFocused()) {
            return w1;
    ...
    private Window getRoot(Window win) {
     ...
     */
    private Window getCommonNearestRoot(Window w1, Window w2) {
        Window retval = null;
    ...
    private static class Singleton {
        ...
        private static final DefaultTopWindowFinder1 INSTANCE = new DefaultTopWindowFinder1();
    }
...
*/
         ...
         */
        private boolean isImplicitlyOnTop(boolean alwaysOnTopValue) {
...
/*        public void setParent(WindowWrapper parent) {
            this.parent = parent;
        }
*/
        @Override
