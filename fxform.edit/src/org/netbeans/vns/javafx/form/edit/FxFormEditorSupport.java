...
public final class FxFormEditorSupport extends DataEditorSupport implements FxEditorSupport,
        EditorCookie.Observable, CloseCookie, PrintCookie {
    
    /** ID of the form designer (in the multiview) */
    static final String MV_FORM_ID = "fxform"; //NOI18N
    ...
    static final String iconURL =
            "org/netbeans/vns/javafx/form/edit/resources/form.gif"; // NOI18N
    
    /** The DataObject of the form */
    private FxFormDataObject formDataObject;
    ...
    //private FxFormEditor formEditor;
    
    /** Set of opened FormEditorSupport instances (java or form opened) */
    private static final Set<FxFormEditorSupport> opened = Collections.synchronizedSet(new HashSet<FxFormEditorSupport>());
    ...
    public FxFormEditorSupport(FxFormDataObject formDataObject) {
    ...
    public void openFormEditor(boolean forceFormElement) {
        ...
        Logger.getLogger(FxFormEditorSupport.class.getName()).log(Level.FINER, "Opening form time 1: {0}ms", (System.currentTimeMillis()-ms)); // NOI18N
    }
    ...
    void showOpeningStatus(String fmtMessage) {
        ...
        StatusDisplayer.getDefault().setStatusText(fmtMessage);
...
*/        
        ...
        if (fsl == null) {
            fsl = new FileStatusListener() {
                ...
                public void annotationChanged(FileStatusEvent ev) {
                    synchronized (opened) {
                        Iterator<FxFormEditorSupport> iter = opened.iterator();
                        while (iter.hasNext()) {
                            FxFormEditorSupport fes = iter.next();
    ...
    public void openDesign() {
        if (EventQueue.isDispatchThread()) {
            //Util.out("FxFormEditorSupport.openDesign 1");
            openFormEditor(true);
        } else {
            EventQueue.invokeLater(new Runnable() {
                @Override
                public void run() {
                    //Util.out("FxFormEditorSupport.openDesign 2");
                    openFormEditor(true);
                }
            });
        }
    ...
    private void openInAWT() {
        ...
        if (Boolean.TRUE.equals(formDataObject.getPrimaryFile().getAttribute("nonEditableTemplate"))) { // NOI18N
            ...
            String message = " !!! MY openInAWT";
            DialogDisplayer.getDefault().notify(new NotifyDescriptor.Message(message));
            return;
        }
        elementToOpen = JAVA_ELEMENT_INDEX;
        //Util.out("FxFormEditorSupport.openInAWT 1");
        ...
        MultiViewHandler handler = MultiViews.findMultiViewHandler(multiviewTC);
        //Util.out("FxFormEditorSupport.openInAWT 2 handler=" + handler);
    ...
    public void openAt(PositionRef pos) {
        elementToOpen = JAVA_ELEMENT_INDEX;
        //Util.out("FxFormEditorSupport.openAt");
    ...
    boolean startFormLoading() {
        //Util.out("FxFormEditorSupport.startFormLoading");
...
*/        
    ...
    boolean loadOpeningForm() {
        ...
        postCreationUpdate1();
        //Util.out("FxFormEditorSupport.loadOpeningForm");
    ...
    public boolean loadForm() {
        Util.out("FxFormEditorSupport.loadForm");
        return true;
    ...
    public boolean isOpened() {
        //Util.out("FxFormEditorSupport.isOpened");
    ...
    public void saveDocument() throws IOException {
        ...
        Util.out("FxFormEditorSupport.saveDocument 1");
...
//            }
        ...
        //Util.out("FxFormEditorSupport.saveDocument Exception " + ex.getMessage());
    ...
    public void saveSourceOnly() throws IOException {
        try {
            saving = true; // workaround for bug 75225
       // Util.out("FxFormEditorSupport.saveSourceOnly ");
    ...
    private void reportErrors() {
       // Util.out("FxFormEditorSupport.reportErrors ");
...
*/            
    ...
    public final FxFormDataObject getFormDataObject() {
...
*/    
/*    FxFormEditor getFormEditor(boolean initialize) {
        if ((formEditor == null) && initialize) {
            formEditor = new FxFormEditor(formDataObject, this);
        }
        return formEditor;
    }
*/
...
*/
    ...
    public void markModified() {
       // Util.out("FxFormEditorSupport.markModified ");
...
//        }
    ...
    public void reloadForm() {
        Util.out("FxFormEditorSupport.reloadForm canClose = " + canClose());
    ...
    protected org.openide.util.Task reloadDocument() {
        //Util.out("FxFormEditorSupport.reloadDocument 1");
        
        if (multiviewTC == null) {
            //Util.out("FxFormEditorSupport.reloadDocument 2");
            
            return super.reloadDocument();
        }
        //Util.out("FxFormEditorSupport.reloadDocument 3");
        
        org.openide.util.Task docLoadTask = super.reloadDocument();
        
        if (saving) {// workaround for bug 75225
            //Util.out("FxFormEditorSupport.reloadDocument 4");
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            @Override
            public void run() {
                 //Util.out("FxFormEditorSupport.reloadDocument 5");
     ...
     */
/*    public FxFormEditor reloadFormEditor() {
        if (!isOpened()) {
            return null;
        }
        //FormDesignerTC designerTC = getFormDesignerTC();
...
*/                
    ...
    protected boolean notifyModified () {
            //Util.out("FxFormEditorSupport.notifyModified 2");
...
//                            FormUtils.getBundleString("TITLE_UserQuestion"), // NOI18N
    ...
    protected void notifyUnmodified () {
            //Util.out("FxFormEditorSupport.notifyUnmodified 2");
    ...
    private static void attachTopComponentsListener() {
            //Util.out("FxFormEditorSupport.attachTopComponentsListener 1");
        
        if (topcompsListener != null) {
            //Util.out("FxFormEditorSupport.attachTopComponentsListener 2");
 ...
 //Util.out("FxFormEditorSupport.attachTopComponentsListener 3");                    
                            ...
                            if (!newSet.contains(o)) {
                                if (o instanceof CloneableTopComponent) {
                                    ...
                                    if (getSelectedElementType(closedTC) != -1) { // it is our multiview
                                        FxFormEditorSupport fes = getFormEditor(closedTC);
                                        if (fes != null) {
                    ...
                    if (active!=null && getSelectedElementType(active) != -1) { // it is our multiview
                        FxFormEditorSupport fes = getFormEditor(active);
                        if (fes != null) {
                            fes.updateMVTCDisplayName();
                        }
                    }
...
/*        if (formEditor.postCreationUpdate()) {
            try {
                ...
                Logger.getLogger(FxFormEditorSupport.class.getName()).log(Level.INFO, ex.getLocalizedMessage(), ex);
            }
        }
*/        
    }
...
*/
            ...
            if (positions[0] != -1) {
                try {
                    ...
                    Logger.getLogger(FxFormEditorSupport.class.getName()).log(Level.INFO, blex.getLocalizedMessage(), blex);
                }
            }
        }
 ...
 Util.out("FxFormEditorSupport.createPane 1");        
        if (!formDataObject.isValid()) {
Util.out("FxFormEditorSupport.createPane 2");        
             
            return super.createPane(); // Issue 110249
        } 
Util.out("FxFormEditorSupport.createPane 3");        
         
        CloneableTopComponent mvtc = MultiViews.createCloneableMultiView("text/x-fxform", getDataObject());
        ...
        if (editorMode != null) {
Util.out("FxFormEditorSupport.createPane 4");        
    ...
    private static boolean readOnly(FxFormDataObject formDataObject) {
        if (!formDataObject.getPrimaryFile().canWrite()) {
            return true;
        }
        TopComponent active = TopComponent.getRegistry().getActivated();
        if (active != null && getSelectedElementType(active) == FORM_ELEMENT_INDEX) {
            FxFormEditorSupport fes = (FxFormEditorSupport)formDataObject.getFormEditorSupport();
            if (fes != null) {
...
*/                
    ...
    private static String getMVTCToolTipText(FxFormDataObject formDataObject) {
     ...
     */
    private static String[] getMVTCDisplayName(FxFormDataObject formDataObject) {
        Node node = formDataObject.getNodeDelegate();
...
*/            
    ...
    final CloseOperationState canCloseElement(TopComponent tc) {
        ...
        if (!FxFormEditorSupport.isLastView(tc)) {
    ...
    public static FxFormEditorSupport getFormEditor(TopComponent tc) {
        Object dobj = tc.getLookup().lookup(DataObject.class);
        FxFormEditorSupport fes = null;
        if (dobj instanceof FxFormDataObject) {
            FxFormDataObject formDataObject = (FxFormDataObject)dobj;
            fes = (FxFormEditorSupport)formDataObject.getFormEditorSupport();
        }
        return fes;
    }
    ...
    public SimpleSection getInitComponentSection() {
        return getGuardedSectionManager().findSimpleSection(SECTION_INIT);
    }
...
//            || ClassPathUtils.checkUserClass("org.openide.awt.Mnemonics", srcFile); // NOI18N
    ...
    public static class JavaMultiViewEditorElement extends MultiViewEditorElement {
        ...
        private transient FxFormEditorSupport javaEditor;
        ...
        public JavaMultiViewEditorElement(Lookup context) {
            ...
            javaEditor = context.lookup(FxFormEditorSupport.class);
            if (javaEditor != null) {
        ...
        public void componentShowing() {
            ...
            if (dataObject instanceof FxFormDataObject) {
                FxFormDataObject formDO = (FxFormDataObject) dataObject;
                FxFormEditorSupport fe = (FxFormEditorSupport)formDO.getFormEditorSupport();
                if (fe != null) {
...
*/
    ...
    private static final class Environment extends DataEditorSupport.Env {
        ...
        protected FileLock takeLock() throws java.io.IOException {            
            return ((FxFormDataObject) getDataObject()).getPrimaryEntry().takeLock();
        }
        
        @Override
        public CloneableOpenSupport findCloneableOpenSupport() {
            return this.getDataObject().getCookie(FxFormEditorSupport.class);
        }
...
//            } else 
            ...
            if (EventQueue.isDispatchThread()) {
                ...
                try {
                    ...
                    Logger.getLogger(FxFormEditorSupport.class.getName()).log(Level.INFO, "", ex); // NOI18N
