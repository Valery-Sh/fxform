...
public class FxClassPathUtils {

    /**
     * Cache the {@code classloader} instance related to the project (shared for all
     * forms from the project).
     */
    private static Map<Project,Reference<FxFormClassLoader>> loaders = new WeakHashMap<Project,Reference<FxFormClassLoader>>();

    /**
     * Make sure the project-related  {@code classloader} is not forgotten by the weak
     ...
     */
    private static Map<FileObject, FxFormClassLoader> formLoaders = new HashMap<FileObject,FxFormClassLoader>();

    /**
     * Class loading type for a class to be always loaded by the IDE's system
     *  {@code classloader} (i.e. from a module). The system  {@code classloader} is used even if
     * the class was requested by a class that was loaded by the project
     *  {@code classloader}. This can also be used for user classes from project to link
     ...
     * that also includes the project {@code classpath}. Can be used for special design
     * module classes that need to access project {@code classpath} (classes or resources).
     */
     ...
     * project's execution {@code classpath} unless it is a basic JDK class, or a class
     * registered as a support (system) class.
     */
    ...
    private static FxFormClassLoader getFormClassLoader(FileObject fileInProject) {
        Project p = FileOwnerQuery.getOwner(fileInProject);
        Reference<FxFormClassLoader> ref = loaders.get(p);
        FxFormClassLoader fcl = ref != null ? ref.get() : null;
        ClassLoader existingProjectCL = fcl != null ? fcl.getProjectClassLoader() : null;
        ClassLoader newProjectCL = FxProjectClassLoader.getUpToDateClassLoader(
                                     fileInProject, existingProjectCL);
        if (fcl == null || newProjectCL != existingProjectCL) {
            fcl = new FxFormClassLoader(newProjectCL);
            formLoaders.put(fileInProject, fcl);
            loaders.put(p, new WeakReference<FxFormClassLoader>(fcl));
...
*/
    static String getBundleString(String key) {
        return NbBundle.getBundle(FxClassPathUtils.class).getString(key);
    }
    ...
    private static final String CL_LAYER_BASE = "org-netbeans-vns-javafx-demo/classloader/"; // NOI18N
